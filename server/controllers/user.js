const User = require('../models/user');
const bcrypt = require('bcrypt');

async function handleUserSignup(req, res) {
    try {
        if (req.role !== 'admin') {
            return res.status(403).json({ message: 'Forbidden' });
        }

        const { userId, password } = req.body;

        if (!userId || !password) {
            return res.status(400).json({ message: 'All fields are required' });
        }

        const duplicateUser = await User.findOne({ userId }).lean().exec();

        if (duplicateUser) {
            return res.status(409).json({ message: 'User already exists, sign in' });
        }

        const hashedPassword = await bcrypt.hash(password, 10);

        const newUser = await User.create({
            userId,
            password: hashedPassword,
        });

        if (newUser) {
            return res.status(201).json({ message: `New user ${userId} created` });
        }

        return res.status(500).json({ message: 'Failed to create user' });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}

async function getAllUsers(req, res) {
    try {
        if (req.role !== 'admin') {
            return res.status(403).json({ message: 'Forbidden' });
        }

        const users = await User.find().skip(1);

        if (!users || users.length === 0) {
            return res.status(404).json({ message: 'No users found' });
        }

        res.status(200).json(users);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}


async function getTwoUsers(req, res) {
    try {
        if (req.role !== 'admin') {
            return res.status(403).json({ message: 'Forbidden' });
        }

        const users = await User.find().skip(1).limit(2);

        if (!users || users.length === 0) {
            return res.status(404).json({ message: 'No users found' });
        }

        res.status(200).json(users);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}

async function getUserById(req, res) {
    //user will be able to view his details only
    try {
        const userId = req.userId;

        const user = await User.findOne({userId})

        if (!user || user.length === 0) {
            return res.status(404).json({ message: 'No user found' });
        }

        res.status(200).json(user);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}


async function verifyUser(req, res) {
    //only admin will be able to verify user
    try {
        if (req.role !== 'admin') {
            return res.status(403).json({ message: 'Forbidden' });
        }

        const userId = req.params.id;
        const { verified } = req.body;

        const updatedUser = await User.findOneAndUpdate({ userId }, { verified }, { new: true });

        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }

        res.status(200).json({ message: `User ${updatedUser.userId} updated` });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}

defaultImage = "data:image/png;base64,"

async function resetUser(req, res) {
    //only admin will be able to verify user
    try {
        if (req.role !== 'admin') {
            return res.status(403).json({ message: 'Forbidden' });
        }

        const userId = req.params.id;

        const defaultValues = {
            verified: 'no',
            name : '-',
            userImage : defaultImage
        };

        const updatedUser = await User.findOneAndUpdate({ userId }, defaultValues, { new: true });

        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }

        res.status(200).json({ message: `User ${updatedUser.userId} updated` });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}

async function updateUserDetails(req, res) {
    //user will be able to update his details only
    try {
        const userId = req.userId;
        const { name, userImage } = req.body;

        const updatedUser = await User.findOneAndUpdate({ userId }, { name, userImage, verified: 'notYet' }, { new: true });

        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }

        res.status(200).json({ message: `User ${updatedUser.userId} updated` });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Something went wrong' });
    }
}



module.exports = {
    handleUserSignup,
    verifyUser,
    updateUserDetails,
    getAllUsers,
    getTwoUsers,
    getUserById,
    resetUser
};
